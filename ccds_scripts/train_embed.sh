...
model_name=${12}
accelerate_launch=${13} #* use accelerate launch for multi-gpu training
out_dir=${14}
deepspeed_config=${15}
...
./ccds/training/train_scripts/train/generic_lora_train.sh "$DATA_DIR" "$MODEL_PATH" "$DS_PERCENTAGE" "$DATA_SEED" \
"$JOB_NAME" "$VALIDATION_TASK" "$include_validation" "$number_of_gpus" "$TRAIN_FILES" \
"$batch_size_per_device" "$data_shuffle" "$learning_rate" "$num_train_epochs" \
